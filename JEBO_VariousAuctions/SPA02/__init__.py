...
class C(BaseConstants):
    NAME_IN_URL = 'second-price_auction(SPA)'
    PLAYERS_PER_GROUP = 7
    NUM_ROUNDS = 2
    ENDOWMENT = 5000
...
class BidWaiting(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        if len(G1)>1:
            G1[0].is_winner = True
            G1[0].Actual_BuyerBid = G1[1].BuyerBid
        elif len(G1) == 1:
            G1[0].is_winner = True
            G1[0].Actual_BuyerBid = Seller1.SellerReservationPrice
        else:
            pass

        if len(G2)>1:
            G2[0].is_winner = True
            G2[0].Actual_BuyerBid = G2[1].BuyerBid
        elif len(G2) == 1:
            G2[0].is_winner = True
            G2[0].Actual_BuyerBid = Seller2.SellerReservationPrice
        else:
            pass
